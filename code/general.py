...
#28 Jan 24
def parse_dict(json_dict,parents=[],final_kv_list=[]): #create a recursive, linear list of keys/values from a nested dict/json
  for k, v in json_dict.items():
    ...
    if isinstance(v, dict):
      ...
      final_kv_list=parse_dict(v,new_parents, final_kv_list)
    else:
...
def get_json_used_vals_lines(multiline_json_fpath,key,decompress_line=False,max_n_lines=None,use_line_dict=False):
  ...
  for line_i,line0 in enumerate(json_fopen):
    if line_i==max_n_lines: break
    if decompress_line: json_dict=json.loads(decompress(line0))
    else: json_dict=json.loads(line0)
    cur_val=json_dict.get(key)
    if use_line_dict: used_vals_dict[cur_val]=json_dict
    else: used_vals_dict[cur_val]=True
  json_fopen.close()
...
########################## ZIP FILES AND COMPRESSION ##################
###Utilities - zip files functions copy to general soon

import os, tempfile, json, zipfile, zlib

#https://stackoverflow.com/questions/77902559/python-safe-string-compression-without-line-breaks/77928084
def compress(string0) -> bytes:
    ...
    s=zlib.compress(string0.encode())

    return s.replace(b'\\', b'\\\\').replace(b'\n', b'\\n')

def decompress(compressed0):
  ...
  decoded0=re.sub(br'\\(.)', lambda m: b'\\' if m[1] == b'\\' else b'\n', compressed0)
  return zlib.decompress(decoded0)
